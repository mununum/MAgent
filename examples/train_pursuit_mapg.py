...
import magent
from magent.builtin.tf_model import DeepQNetwork, AdvantageActorCritic
from magent.builtin.tf_model import MAPolicyGradient
...
def play_a_round(env, map_size, handles, models, print_every, train=True, render=False, eps=None):
    ...
    while not done:
        ...
        for i in range(n):
            ...
            models[i].infer_action(obs[i], ids[i], 'e_greedy', eps, block=False, state=state)
        for i in range(n):
...
if __name__ == "__main__":
    ...
    for i in range(len(names)):
        ...
        # ))
        models.append(magent.ProcessingModel(
            env, handles[i], names[i], 20000+i, 4000, MAPolicyGradient,
            use_global_state=True, batch_size=512
        ))
